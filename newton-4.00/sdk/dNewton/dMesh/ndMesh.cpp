...
ndMatrix ndMesh::CalculateLocalMatrix(ndVector& sizeOut) const
{
	...
	if (meshEffect)
	{
		const ndMatrix transform(GetGeometryMatrix());
...
{
	ndVector size;
	const ndMatrix localMatrix(CalculateLocalMatrix(size));
...
ndSharedPtr<ndJointBilateralConstraint> ndMesh::CreateJoint()
{
	...
	{
		joint = ndSharedPtr<ndJointBilateralConstraint>(new ndJointRoller());
	}
	else if (strstr(name, "-cylindrical"))
	{
		joint = ndSharedPtr<ndJointBilateralConstraint>(new ndJointCylinder());
	}
	...
	{
		joint = ndSharedPtr<ndJointBilateralConstraint>(new ndJointPlane());
	}
...
ndSharedPtr<ndMeshJoint> ndMesh::LoadJoint(const nd::TiXmlElement* const xmlJoint) const
{
	...
	else if (strcmp(constructor, ndMultiBodyVehicleTireJoint::StaticClassName()) == 0)
	{
		joint = ndSharedPtr<ndMeshJoint>(new ndMeshJointVehicleTireJoint(mesh));
	}
	...
	else if (strcmp(constructor, ndMultiBodyVehicleTorsionBar::StaticClassName()) == 0)
	{
		joint = ndSharedPtr<ndMeshJoint>(new ndMeshJointVehicleTorsionBar(mesh));
	}
