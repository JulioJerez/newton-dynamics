...
class ndAabbPolygonSoup: public ndPolygonSoupDatabase
{
	...
	D_CORE_API void Serialize(ndArray<ndVector>& points, ndArray<ndInt32>& indices, ndArray<ndNode>& nodes) const;
...
/// Get the root node of the hierarchy.
inline ndAabbPolygonSoup::ndNode* ndAabbPolygonSoup::GetRootNode() const
{
	return m_aabb;
}
...
/// Returns the back child node of the hierarchy.
/// Return nullptr if node was a leaf.
inline ndAabbPolygonSoup::ndNode* ndAabbPolygonSoup::GetBackNode(const ndNode* const node) const
{
...
/// Returns the front child node of the hierarchy.
/// Return nullptr if node was a leaf.
inline ndAabbPolygonSoup::ndNode* ndAabbPolygonSoup::GetFrontNode(const ndNode* const node) const
{
	return node->m_right.IsLeaf() ? nullptr : node->m_right.GetNode(m_aabb);
}

/// Returns the bounding box of node in point p0 and p1.
inline void ndAabbPolygonSoup::GetNodeAabb(const ndNode* const node, ndVector& p0, ndVector& p1) const
{
