...
inline ndInt32 cpuKernelRoundoff(ndInt32 value, ndInt32 workgroupSize)
{
...
class brainLayerReluActivation : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        ndInt64 outputOffset = inputOffset + cpuKernelRoundoff(inputSize, workGroupSize);
...
class brainLayerLeakyReluActivation : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        for (ndInt32 i = 0; i < modWorkGroupSize; i += workGroupSize)
        {
            ...
            {
                ndBrainFloat inputValue = inputOutputData[inputOffset + i + itemId];
                ndBrainFloat outputValue = (inputValue >= ndBrainFloat(0.0f)) ? inputValue : ND_BRAIN_LAYER_ACTIVATIONLEAKY_LRU_GRADIENT * inputValue;
                inputOutputData[outputOffset + i + itemId] = outputValue;
            }
        }
        for (ndInt32 itemId = 0; itemId < workGroupSizeReminder; ++itemId)
...
class brainLayerBrainReluBackPropagate : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        ndInt64 srcBase = groupId * ndInt64(inputOutputSize) + inputOutputStartOffset;
        ndInt64 dstBase = srcBase + cpuKernelRoundoff(inputSize, workGroupSize);
        ndAssert(srcBase >= 0);
        ndAssert(dstBase >= 0);
...
class brainLayerBrainLeakyReluBackPropagate : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        for (ndInt32 i = 0; i < modWorkGroupSize; i += workGroupSize)
        {
            ...
            {
                ndBrainFloat inpuData = inputOutputData[srcBase + i + itemId];
                ndBrainFloat gradient = (inpuData >= ndBrainFloat(0.0f)) ? ndBrainFloat(1.0f) : ND_BRAIN_LAYER_ACTIVATIONLEAKY_LRU_GRADIENT;
                ndBrainFloat outputGrad = inputOutputGradients[dstBase + i + itemId];
                inputOutputGradients[srcBase + i + itemId] = gradient * outputGrad;
            }
        }
...
class brainLayerBrainTanhBackPropagate : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        const ndBrainMemVector output(&inputOutputData[dstBase], inputSize);
        const ndBrainMemVector outputDerivative(&inputOutputGradients[dstBase], inputSize);
        ndBrainMemVector inputDerivative(&inputOutputGradients[srcBase], inputSize);
        ...
        for (ndInt32 i = 0; i < modWorkGroupSize; i += workGroupSize)
        {
            for (ndInt32 itemId = 0; itemId < workGroupSize; ++itemId)
            {
                ndBrainFloat outputData = output[i + itemId];
                ndBrainFloat a = ndBrainFloat(1.0f) - outputData * outputData;
                ndBrainFloat b = outputDerivative[i + itemId];
                inputDerivative[i + itemId] = a * b;
            }
        }
        for (ndInt32 itemId = 0; itemId < workGroupSizeReminder; ++itemId)
        {
            ndBrainFloat outputData = output[modWorkGroupSize + itemId];
            ndBrainFloat a = ndBrainFloat(1.0f) - outputData * outputData;
            ndBrainFloat b = outputDerivative[modWorkGroupSize + itemId];
            inputDerivative[modWorkGroupSize + itemId] = a * b;
        }

        #ifdef _DEBUG
        {
            const ndBrainMemVector checkDerivative(&inputOutputGradients[srcBase], workGroupSize);
            ndInt32 padded = (inputSize + workGroupSize - 1) & -workGroupSize;
            for (ndInt32 i = inputSize; i < padded; ++i)
            {
                ndBrainFloat a = checkDerivative[i];
                ndAssert(a == ndBrainFloat(0.0f));
            }
        }
        #endif
...
class brainLayerBrainLinearBackPropagate : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ndBrainUniformBuffer* const buffer0 = (ndBrainUniformBuffer*)m_parameters[0];
        ndBrainFloatBuffer* const buffer3 = (ndBrainFloatBuffer*)m_parameters[3];
        ndBrainFloatBuffer* const buffer6 = (ndBrainFloatBuffer*)m_parameters[6];

        ndBrainFloat* const slopesPtr = (ndBrainFloat*)buffer6->GetGpuBuffer()->GetPtr();
        ndBrainFloat* const inputOutputGradients = (ndBrainFloat*)buffer3->GetGpuBuffer()->GetPtr();
        ndCommandSharedInfo* const parameters = (ndCommandSharedInfo*)buffer0->GetGpuBuffer()->GetPtr();
        ...
        const ndBrainMemVector slopes(slopesPtr, inputSize);
        const ndBrainMemVector outputDerivative(&inputOutputGradients[dstBase], inputSize);
        ndBrainMemVector inputDerivative(&inputOutputGradients[srcBase], inputSize);
        ...
        for (ndInt32 i = 0; i < modWorkGroupSize; i += workGroupSize)
        {
            for (ndInt32 itemId = 0; itemId < workGroupSize; ++itemId)
            {
                ndBrainFloat slope = slopes[i + itemId];
                ndBrainFloat outputGrad = outputDerivative[i + itemId];
                inputDerivative[i + itemId] = slope * outputGrad;
            }
        }
        for (ndInt32 itemId = 0; itemId < workGroupSizeReminder; ++itemId)
        {
            ndBrainFloat slope = slopes[modWorkGroupSize + itemId];
            ndBrainFloat outputGrad = outputDerivative[modWorkGroupSize + itemId];
            inputDerivative[modWorkGroupSize + itemId] = slope * outputGrad;
        }
...
class brainLayerMatrixMatrixMultiply : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        ndAssert((inputSize + ND_GPU_TILED_MATRIX_ROWS - 1) / ND_GPU_TILED_MATRIX_ROWS == 1);
        ...
        ndInt32 inputOffset = inputBase;
        ndInt32 weightOffset = weightsBase;
        for (ndInt32 k = 0; k < ND_GPU_TILED_MATRIX_ROWS * ND_GPU_TILED_MATRIX_ROWS; k += workGroupSize)
        {
            const ndInt32 base = k >> ND_GPU_TILED_MATRIX_ROWS_BITS;
            for (ndInt32 itemId = 0; itemId < workGroupSize; ++itemId)
        ...
        for (ndInt32 j1 = 0; j1 < ND_GPU_TILED_MATRIX_ROWS; j1 += ND_GPU_TILED_MATRIX_ROWS / 2)
        {
            for (ndInt32 i1 = 0; i1 < ND_GPU_TILED_MATRIX_ROWS; i1 += ND_GPU_TILED_MATRIX_ROWS / 2)
            {
                for (ndInt32 itemId = 0; itemId < workGroupSize; ++itemId)
                {
                    ...
                    smallTile[itemIdSmallTile_j][itemIdSmallTile_i] = ndBrainFloat(0.0f);
                }

                for (ndInt32 m = 0; m < ND_GPU_TILED_MATRIX_ROWS; m += ND_GPU_TILED_MATRIX_ROWS / 2)
                {
                    for (ndInt32 k = 0; k < ND_GPU_TILED_MATRIX_ROWS / 2; ++k)
                    {
                        for (ndInt32 itemId = 0; itemId < workGroupSize; ++itemId)
                        {
                            ndInt32 itemIdSmallTile_i = itemId & (ND_GPU_TILED_MATRIX_ROWS / 2 - 1);
                            ndInt32 itemIdSmallTile_j = itemId >> (ND_GPU_TILED_MATRIX_ROWS_BITS - 1);
                            ndBrainFloat input = tile_inputs[j1 + itemIdSmallTile_j][m + k];
                            ndBrainFloat weight = tile_weights[i1 + itemIdSmallTile_i][m + k];
                            smallTile[itemIdSmallTile_j][itemIdSmallTile_i] += weight * input;
                        }
                    }
                }
...
class brainLayerBrainBackPropagateMatrixBiasGradients : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32 workGroupSize) override
    {
        ...
        const ndInt64 outputGradientOffset = inputGradientOffset + cpuKernelRoundoff(inputSize, workGroupSize);

        const ndInt32 width = (inputSize + ND_GPU_TILED_MATRIX_ROWS - 1) & -ND_GPU_TILED_MATRIX_ROWS;
        const ndInt32 height = (outputSize + ND_GPU_TILED_MATRIX_ROWS - 1) & -ND_GPU_TILED_MATRIX_ROWS;

        const ndInt32 matrixSize = cpuKernelRoundoff(width * height, ND_DEFAULT_WORKGROUP_SIZE);;
...
class brainLayerBrainBackPropagateMatrixWeightsGradients : public ndBrainKernel
{
    ...
    void Execute(ndInt32 groupId, ndInt32) override
    {
        ...
        const ndInt32 dimK = info.m_matrixDimensionK;
