...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int maxSize)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const float w = (flags & ImGuiComboFlags_MaxSize) ? maxSize : CalcItemWidth();
