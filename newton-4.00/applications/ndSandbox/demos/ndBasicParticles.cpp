...
class MarchingCubeParticles : public ndDemoEntityManager::OnPostUpdate
{
	public:
	...
	MarchingCubeParticles(ndDemoEntityManager* const scene)
		:OnPostUpdate()
		,m_isoSurface(scene)
	{
...
void ndBasicParticles(ndDemoEntityManager* const scene)
{
	...
	const ndVector origin(0.0f, 0.5f, 0.0f, 1.0f);

	ndVector floor(FindFloor(*scene->GetWorld(), origin, 400.0f));
	ndMatrix originMatrix(ndCalculateMatrix(rot, floor));

	ndSharedPtr<ndDemoEntityManager::OnPostUpdate>marchingCubeMesh(new MarchingCubeParticles(scene));
