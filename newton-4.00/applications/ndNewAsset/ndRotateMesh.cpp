...
//
// *************************************************************
ndRotatePivots::ndRotatePivots(ndAssetEditor* const owner)
	:ndAssetTool(owner)
{
	m_angles[0] = ndReal(0.0f);
	m_angles[1] = ndReal(0.0f);
...
void ndRotatePivots::ApplyRotation()
{
	const ndMatrix rotation(ndPitchMatrix(m_angles[0] * ndDegreeToRad) * ndYawMatrix(m_angles[1] * ndDegreeToRad) * ndRollMatrix(m_angles[2] * ndDegreeToRad));
	ndMesh* const mesh = *m_owner->GetMesh();
	ndAssert(mesh);
	mesh->ApplyPivotsRotation(rotation);
...
void ndRotatePivots::Execute()
{
	...
	bool toolActive = m_owner->GetActiveTool();
	ImGui::Begin("rotate pivots", &toolActive, flags);
	m_owner->SetActiveTool(toolActive);
