...
{
	public:
	ndUndoRedoStructuralJoint(ndAssetEditor* const editor, const ndMesh* const selectedNode)
		:ndUndoRedoCommand(editor, selectedNode)
		,m_joint(ndSharedPtr<ndMeshJoint>(selectedNode->GetJoint()->Duplicate()))
	{
	}
...
void ndAssetEditor::EditHingeJoint()
{
	EditJointGlobalMatrix();
...
void ndAssetEditor::EditSliderJoint()
{
	EditJointGlobalMatrix();
...
void ndAssetEditor::EditDifferentialJoint()
{
	...
	if (ImGui::InputFloat("limited slip omega", &value, 0.0f, 0.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
	{
		m_undoRedo.Push(ndSharedPtr<ndUndoRedoCommand>(new ndUndoRedoStructuralJoint(this, *m_currentSelection)));
...
void ndAssetEditor::EditMotorJoint()
{
	...
	if (ImGui::InputFloat("fuel Injector", &fuel, 0.0f, 0.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
	{
		fuel = ndAbs(fuel);
		m_undoRedo.Push(ndSharedPtr<ndUndoRedoCommand>(new ndUndoRedoStructuralJoint(this, *m_currentSelection)));
		engineCurve.m_omegaStep = fuel;
		m_undoRedo.Push(ndSharedPtr<ndUndoRedoCommand>(new ndUndoRedoStructuralJoint(this, *m_currentSelection)));
	}
